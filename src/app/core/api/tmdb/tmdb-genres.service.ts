...
import { TmdbGenreListDto } from '../../models/tmdb/genres/genre.dto';
...
export class TmdbGenresService {
     ...
     */
    getMovieGenres(language: string = 'en'): Observable<TmdbGenreListDto> {
        const params = new HttpParams().set('language', language);
        return this.http.get<TmdbGenreListDto>('/genre/movie/list', { params });
    }
     ...
     * @param language Optional ISO 639-1 language code (defaults to 'en').
     */
    getTvGenres(language: string = 'en'): Observable<TmdbGenreListDto> {
        const params = new HttpParams().set('language', language);
        return this.http.get<TmdbGenreListDto>('/genre/tv/list', { params });
